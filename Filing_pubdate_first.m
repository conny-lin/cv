...
%% EXTRACT FILE COMPONENTS - DESCRIPTOR
fprintf('\n\nGetting type code...');
% set output array
typeNames = cell(size(A,1),1);
...
for i = 1:size(FileNameTable,1)
   ...
   filename_new = sprintf('%s %s %s %s %s',...
       FileNameTable.date{i}, ...
       FileNameTable.sector{i},...
       FileNameTable.type{i},...
       FileNameTable,descripter{i},...
       FileNameTable.ext{i});
